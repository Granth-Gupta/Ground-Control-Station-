...
function computeProfile(t) {
  ...
  if (t <= ASCENT_TIME) {
    ...
    alt = MAX_ALT * (1 - f);
    vSpeed = state.chuteDeployed
      ? -9 - (Math.random() * 0.6 - 0.3)
      : -(6 + 18 * f);
    phase = "DESCENT";
  } else {
    alt = 0;
...
function barometric(altM) {
  return 101.325 * Math.pow(1 - 2.25577e-5 * altM, 5.25588);
}
...
function startSim() {
  ...
  state.running = true;
  if (!state.startTime)
    state.startTime = Date.now() - state.missionSeconds * 1000;
...
function tick() {
  ...
  state.phase =
    prof.phase === "LANDED" && !state.separated ? "DESCENT" : prof.phase;
  ...
  state.yaw =
    (state.yaw + (prof.phase === "DESCENT" ? 6 : 2) + Math.random() * 2) % 360;

  state.packetCount++;

  const packet = {
    time: fmtTime(t),
    missionSeconds: t.toFixed(1),
    ...
    state: prof.phase,
    payloadAltitude: pAlt.toFixed(1),
    payloadTemp: pTemp.toFixed(1),
    payloadVoltage: pVolt.toFixed(2),
...
function renderTelemetry(p) {
  ...
  document.getElementById(
    "tAlt"
  ).innerHTML = `${p.altitude}<span class="unit">m</span>`;
  document.getElementById(
    "tPress"
  ).innerHTML = `${p.pressure}<span class="unit">kPa</span>`;
  document.getElementById(
    "tTemp"
  ).innerHTML = `${p.temperature}<span class="unit">°C</span>`;
  document.getElementById(
    "tVolt"
  ).innerHTML = `${p.voltage}<span class="unit">V</span>`;
  document.getElementById("tSats").textContent = p.sats;
  document.getElementById("tLatLon").textContent = `${p.lat}, ${p.lon}`;

  document.getElementById(
    "pAlt"
  ).innerHTML = `${p.payloadAltitude}<span class="unit">m</span>`;
  document.getElementById(
    "pTemp"
  ).innerHTML = `${p.payloadTemp}<span class="unit">°C</span>`;
  document.getElementById(
    "pVolt"
  ).innerHTML = `${p.payloadVoltage}<span class="unit">V</span>`;
  document.getElementById(
    "pDescent"
  ).innerHTML = `${p.descentRate}<span class="unit">m/s</span>`;

  const voltEl = document.getElementById("tVolt");
  voltEl.className =
    "v " +
    (parseFloat(p.voltage) < 6.8
      ? "bad"
      : parseFloat(p.voltage) < 7.4
      ? "warn"
      : "");
}
...
function updateErrorCode(p) {
  ...
  setErrLine(
    "e1",
    d1,
    `${d1 ? "outside range (" + p.descentRate + " m/s)" : "within 8–10 m/s"}`
  );
...
function setErrLine(prefix, bad, text) {
  document.getElementById(prefix + "chip").className =
    "chip" + (bad ? " bad" : "");
  ...
  return new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { display: false },
        y: {
          ticks: { color: "#5f7488", font: { size: 8 }, maxTicksLimit: 3 },
          grid: { color: "#16202c" },
        },
      },
...
function pushPoint(chart, label, val) {
  ...
  if (chart.data.labels.length > CH_WINDOW) {
...
}).setView([state.lat, state.lon], 16);
...
camera3.position.set(3.4, 2.2, 3.4);
...
const bodyGeo = new THREE.BoxGeometry(1.4, 0.6, 1.4);
...
const wireGeo = new THREE.BoxGeometry(1.42, 0.62, 1.42);
const wire = new THREE.LineSegments(
  new THREE.EdgesGeometry(wireGeo),
  new THREE.LineBasicMaterial({ color: 0x2ad6ff })
);
satGroup.add(wire);
...
[
  ...
  fin.position.set(pos[0], 0, pos[2]);
  satGroup.add(fin);
});

const noseGeo = new THREE.ConeGeometry(0.14, 0.4, 8);
...
function resizeThree() {
  const w = cubeWrap.clientWidth,
    h = cubeWrap.clientHeight;
  if (w < 10 || h < 10) return;
  camera3.aspect = w / h;
...
function updateOrientation(p) {
  const roll = parseFloat(p.roll),
    pitch = parseFloat(p.pitch),
    yaw = parseFloat(p.yaw);
  ...
  hzRotor.setAttribute(
    "transform",
    `translate(100 90) rotate(${-roll}) translate(-100 ${-90 + pitch * 1.4})`
  );
...
function logCmd(msg, level = "ok") {
  ...
  entry.innerHTML = `<span class="t">${new Date().toLocaleTimeString(
    "en-GB"
  )}</span>${msg}`;
  el.prepend(entry);
...
document.getElementById("btnExportCsv").addEventListener("click", () => {
  if (state.log.length === 0) {
    logCmd("Export CSV: no telemetry logged yet", "warn");
    return;
  }
  const headers = Object.keys(state.log[0]);
  const rows = [headers.join(",")].concat(
    state.log.map((r) => headers.map((h) => r[h]).join(","))
  );
